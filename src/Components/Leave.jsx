...
class Leave extends Component {
  ...
  handleReject = async (id) => {
    try {
      await axios.post(`http://localhost:8080/reject/${id}`);

      const updatedLeave = this.state.pendingLeave.filter((leave) => leave.id !== id);
      this.setState({ pendingLeave: updatedLeave });
      alert(`Leave with ID ${id} rejected.`);
    } catch (error) {
              ...
              <tbody>
                {pendingLeave.map((leave) => (
                  <tr key={leave.id} style={{ backgroundColor: '#fff' }}>
                    <td style={{ border: '1px solid #ddd', padding: '8px', fontFamily: 'Times New Roman' }}>{leave.id}</td>
                    <td style={{ border: '1px solid #ddd', padding: '8px', fontFamily: 'Times New Roman' }}>{leave.empId}</td>
                    ...
                    <td style={{ border: '1px solid #ddd', padding: '8px', fontFamily: 'Times New Roman' }}>{leave.startDate}</td>
                    <td style={{ border: '1px solid #ddd', padding: '8px', fontFamily: 'Times New Roman' }}>{leave.endDate}</td>
